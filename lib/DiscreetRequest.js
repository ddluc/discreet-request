...
class DiscreetRequest {
  ...
  /**
   * @description gets a fill proxy url
  ...
  getProxyUrl() {
    ...
    if (proxy !== null) {
      if (this.proxyAuth.username.length > 0 && this.proxyAuth.password.length > 0) {
        fullProxyUrl = `${this.protocol}://${this.proxyAuth.username}:${this.proxyAuth.password}@${proxy}`;
      } else {
        fullProxyUrl = `${this.protocol}://${proxy}`;
      }
      logger.dev(`Creating discreet request with proxy address ${fullProxyUrl}`);
    }
    return fullProxyUrl;
  }
