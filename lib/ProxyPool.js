...
class ProxyPool {
  ...
  constructor(proxies=[], proxyAuth=null, options) {
    ...
    if (testProxies && refreshProxies) {
      this.runProxyTestsOnInterval();
    } else if (testProxies) {
      this.runProxyTests();
    } else {
      this.healthyProxies = proxies;
    }
  }
  ...
  buildProxyUrl(proxy) {
    ...
    if (proxy !== null) {
      if (
          (this.proxyAuth !== null && this.proxyAuth.username.length > 0) &&
          (this.proxyAuth !== null && this.proxyAuth.password.length > 0)
      ) {
